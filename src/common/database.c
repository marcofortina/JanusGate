...
static const char scrub_sensitive_data[] =
    ...
    "DELETE FROM "
    "policy_statistics_rule_hour_budget;"
    "DELETE FROM "
    "policy_statistics_domain_budget;"
    "UPDATE policy_statistics_storage "
    "SET detail_rows=0,estimated_bytes=0,"
    "cardinality_dropped=0,"
    "storage_dropped=0,"
    "storage_suspended=0 WHERE id=1;"
...
static const char preserve_sensitive_data[] =
    ...
    "DELETE FROM main.policy_statistics_rule_hour_budget;"
    "DELETE FROM main.policy_statistics_domain_budget;"
    ...
    "CREATE TEMP TABLE restored_policy_statistics_limits AS SELECT "
    "detail_enabled,detail_max_rows,detail_max_rows_per_rule_hour,"
    "detail_max_domains_per_rule_hour FROM "
    "main.policy_statistics_configuration WHERE id=1;"
    "UPDATE main.policy_statistics_configuration SET detail_enabled=1,"
    "detail_max_rows=5000000,detail_max_rows_per_rule_hour=100000,"
    "detail_max_domains_per_rule_hour=10000 WHERE id=1;"
    "UPDATE main.policy_statistics_storage SET detail_rows=0,"
    "estimated_bytes=(SELECT estimated_bytes FROM "
    "retained.policy_statistics_storage WHERE id=1),"
    "cardinality_dropped=(SELECT cardinality_dropped FROM "
    "retained.policy_statistics_storage WHERE id=1),"
    "storage_dropped=(SELECT storage_dropped FROM "
    "retained.policy_statistics_storage WHERE id=1),storage_suspended=0 "
    "WHERE id=1;"
    "INSERT INTO main.policy_impact_buckets "
    "SELECT * FROM retained.policy_impact_buckets;"
    "UPDATE main.policy_statistics_configuration SET "
    "detail_enabled=(SELECT detail_enabled FROM "
    "restored_policy_statistics_limits),detail_max_rows=(SELECT "
    "detail_max_rows FROM restored_policy_statistics_limits),"
    "detail_max_rows_per_rule_hour=(SELECT "
    "detail_max_rows_per_rule_hour FROM restored_policy_statistics_limits),"
    "detail_max_domains_per_rule_hour=(SELECT "
    "detail_max_domains_per_rule_hour FROM "
    "restored_policy_statistics_limits) WHERE id=1;"
    "DROP TABLE restored_policy_statistics_limits;"
...
/** Bound detailed policy-statistics cardinality and storage consumption. */
static const char migration_20_policy_statistics_configuration[] =
    "ALTER TABLE policy_statistics_configuration ADD COLUMN "
    "detail_enabled INTEGER NOT NULL DEFAULT 1 CHECK(detail_enabled IN "
    "(0,1));"
    "ALTER TABLE policy_statistics_configuration ADD COLUMN "
    "detail_max_rows INTEGER NOT NULL DEFAULT 250000 "
    "CHECK(detail_max_rows BETWEEN 1000 AND 5000000);"
    "ALTER TABLE policy_statistics_configuration ADD COLUMN "
    "detail_max_rows_per_rule_hour INTEGER NOT NULL DEFAULT 1000 "
    "CHECK(detail_max_rows_per_rule_hour BETWEEN 10 AND 100000);"
    "ALTER TABLE policy_statistics_configuration ADD COLUMN "
    "detail_max_domains_per_rule_hour INTEGER NOT NULL DEFAULT 256 "
    "CHECK(detail_max_domains_per_rule_hour BETWEEN 10 AND 10000);"
    "ALTER TABLE policy_statistics_configuration ADD COLUMN "
    "maximum_database_bytes INTEGER NOT NULL DEFAULT 1073741824 "
    "CHECK(maximum_database_bytes BETWEEN 67108864 AND 1099511627776);"
    "ALTER TABLE policy_statistics_configuration ADD COLUMN "
    "minimum_free_bytes INTEGER NOT NULL DEFAULT 268435456 "
    "CHECK(minimum_free_bytes BETWEEN 0 AND 1099511627776);"
    "UPDATE policy_statistics_configuration SET retention_months=3 "
    "WHERE id=1 AND revision=1 AND retention_months=12;"
    "CREATE TABLE policy_statistics_storage("
    "id INTEGER PRIMARY KEY CHECK(id=1),"
    "detail_rows INTEGER NOT NULL CHECK(detail_rows>=0),"
    "estimated_bytes INTEGER NOT NULL DEFAULT 0 CHECK(estimated_bytes>=0),"
    "cardinality_dropped INTEGER NOT NULL DEFAULT 0 "
    "CHECK(cardinality_dropped>=0),"
    "storage_dropped INTEGER NOT NULL DEFAULT 0 CHECK(storage_dropped>=0),"
    "storage_suspended INTEGER NOT NULL DEFAULT 0 "
    "CHECK(storage_suspended IN (0,1))"
    ") STRICT;"
    "INSERT INTO policy_statistics_storage(id,detail_rows) "
    "SELECT 1,count(*) FROM policy_impact_buckets;"
    "CREATE TABLE policy_statistics_rule_hour_budget("
    "bucket_start INTEGER NOT NULL,"
    "dimension TEXT NOT NULL CHECK(dimension IN ('domain','destination')),"
    "statistics_id BLOB NOT NULL CHECK(length(statistics_id)=16),"
    "row_count INTEGER NOT NULL CHECK(row_count>0),"
    "PRIMARY KEY(bucket_start,dimension,statistics_id)"
    ") WITHOUT ROWID,STRICT;"
    "INSERT INTO policy_statistics_rule_hour_budget "
    "SELECT bucket_start,dimension,statistics_id,count(*) FROM "
    "policy_impact_buckets GROUP BY bucket_start,dimension,statistics_id;"
    "CREATE TABLE policy_statistics_domain_budget("
    "bucket_start INTEGER NOT NULL,"
    "dimension TEXT NOT NULL CHECK(dimension IN ('domain','destination')),"
    "statistics_id BLOB NOT NULL CHECK(length(statistics_id)=16),"
    "domain TEXT NOT NULL CHECK(length(domain) BETWEEN 1 AND 253),"
    "row_count INTEGER NOT NULL CHECK(row_count>0),"
    "PRIMARY KEY(bucket_start,dimension,statistics_id,domain)"
    ") WITHOUT ROWID,STRICT;"
    "INSERT INTO policy_statistics_domain_budget SELECT bucket_start,"
    "dimension,statistics_id,domain,count(*) FROM policy_impact_buckets "
    "WHERE domain<>'' GROUP BY bucket_start,dimension,statistics_id,domain;";

/** Reject new detailed rows beyond configured storage budgets. */
static const char migration_20_policy_statistics_guards[] =
    "CREATE TRIGGER policy_impact_storage_guard BEFORE INSERT ON "
    "policy_impact_buckets WHEN NOT EXISTS(SELECT 1 FROM "
    "policy_impact_buckets WHERE bucket_start=NEW.bucket_start AND "
    "dimension=NEW.dimension AND statistics_id=NEW.statistics_id AND "
    "path=NEW.path AND client_family=NEW.client_family AND "
    "client_address=NEW.client_address AND client_mac=NEW.client_mac AND "
    "vlan_id=NEW.vlan_id AND domain=NEW.domain AND "
    "query_type=NEW.query_type) AND ((SELECT detail_enabled FROM "
    "policy_statistics_configuration WHERE id=1)=0 OR (SELECT "
    "storage_suspended FROM policy_statistics_storage WHERE id=1)=1) "
    "BEGIN SELECT RAISE(IGNORE); END;"
    "CREATE TRIGGER policy_impact_cardinality_guard BEFORE INSERT ON "
    "policy_impact_buckets WHEN NOT EXISTS(SELECT 1 FROM "
    "policy_impact_buckets WHERE bucket_start=NEW.bucket_start AND "
    "dimension=NEW.dimension AND statistics_id=NEW.statistics_id AND "
    "path=NEW.path AND client_family=NEW.client_family AND "
    ...
    "vlan_id=NEW.vlan_id AND domain=NEW.domain AND "
    "query_type=NEW.query_type) AND (SELECT detail_enabled FROM "
    "policy_statistics_configuration WHERE id=1)=1 AND (SELECT "
    "storage_suspended FROM policy_statistics_storage WHERE id=1)=0 AND "
    "((SELECT detail_rows FROM policy_statistics_storage WHERE id=1)>="
    "(SELECT detail_max_rows FROM policy_statistics_configuration WHERE "
    "id=1) OR coalesce((SELECT row_count FROM "
    "policy_statistics_rule_hour_budget WHERE bucket_start=NEW.bucket_start "
    "AND dimension=NEW.dimension AND statistics_id=NEW.statistics_id),0)>="
    "(SELECT detail_max_rows_per_rule_hour FROM "
    "policy_statistics_configuration WHERE id=1) OR (NEW.domain<>'' AND NOT "
    "EXISTS(SELECT 1 FROM policy_statistics_domain_budget WHERE "
    "bucket_start=NEW.bucket_start AND dimension=NEW.dimension AND "
    "statistics_id=NEW.statistics_id AND domain=NEW.domain) AND (SELECT "
    "count(*) FROM policy_statistics_domain_budget WHERE "
    "bucket_start=NEW.bucket_start AND dimension=NEW.dimension AND "
    "statistics_id=NEW.statistics_id)>=(SELECT "
    "detail_max_domains_per_rule_hour FROM "
    "policy_statistics_configuration WHERE id=1))) BEGIN UPDATE "
    "policy_statistics_storage SET cardinality_dropped=CASE WHEN "
    "cardinality_dropped<9223372036854775807 THEN cardinality_dropped+1 "
    "ELSE cardinality_dropped END WHERE id=1; SELECT RAISE(IGNORE); END;";

/** Maintain exact bounded-detail cardinality counters. */
static const char migration_20_policy_statistics_counters[] =
    "CREATE TRIGGER policy_impact_count_insert AFTER INSERT ON "
    "policy_impact_buckets BEGIN UPDATE policy_statistics_storage SET "
    "detail_rows=detail_rows+1 WHERE id=1; INSERT INTO "
    "policy_statistics_rule_hour_budget(bucket_start,dimension,"
    "statistics_id,row_count) VALUES(NEW.bucket_start,NEW.dimension,"
    "NEW.statistics_id,1) ON CONFLICT DO UPDATE SET row_count=row_count+1; "
    "INSERT INTO policy_statistics_domain_budget(bucket_start,dimension,"
    "statistics_id,domain,row_count) SELECT NEW.bucket_start,NEW.dimension,"
    "NEW.statistics_id,NEW.domain,1 WHERE NEW.domain<>'' ON CONFLICT DO "
    "UPDATE SET row_count=row_count+1; END;"
    "CREATE TRIGGER policy_impact_count_delete AFTER DELETE ON "
    "policy_impact_buckets BEGIN UPDATE policy_statistics_storage SET "
    "detail_rows=max(detail_rows-1,0) WHERE id=1; DELETE FROM "
    "policy_statistics_rule_hour_budget WHERE "
    "bucket_start=OLD.bucket_start AND dimension=OLD.dimension AND "
    "statistics_id=OLD.statistics_id AND row_count=1; UPDATE "
    "policy_statistics_rule_hour_budget SET row_count=row_count-1 WHERE "
    "bucket_start=OLD.bucket_start AND dimension=OLD.dimension AND "
    "statistics_id=OLD.statistics_id; DELETE FROM "
    "policy_statistics_domain_budget WHERE "
    "OLD.domain<>'' AND bucket_start=OLD.bucket_start AND "
    "dimension=OLD.dimension AND statistics_id=OLD.statistics_id AND "
    "domain=OLD.domain AND row_count=1; UPDATE "
    "policy_statistics_domain_budget SET row_count=row_count-1 WHERE "
    "OLD.domain<>'' AND bucket_start=OLD.bucket_start AND "
    "dimension=OLD.dimension AND statistics_id=OLD.statistics_id AND "
    "domain=OLD.domain; END;"
    "INSERT INTO schema_migrations(version,applied_at) VALUES(20,unixepoch());"
    "PRAGMA user_version=20;";

/** Ordered statement groups composing schema version twenty. */
static const char *const migration_20[] = {
    migration_20_policy_statistics_configuration,
    migration_20_policy_statistics_guards,

...
interfaces. On OpenBSD, the pinned libcurl and CivetWeb builds use the same
ports OpenSSL 3 runtime as JanusGate; this avoids loading the conflicting base
LibreSSL API into a process that performs OpenSSL operations. The unprivileged
daemon and Web service use `pledge`; the narrow network helper cannot because
the required bridge and MTU ioctls have no corresponding promise. Those
operating-system facilities are not copied into the repository.

...
the same transaction as a durable webhook outbox item. A short transaction
atomically snapshots the transport and claims an event before delivery.
Verified HTTPS holds no SQLite transaction and does not serialize ordinary
management mutations, while a restore lease prevents state replacement until
the attempt records its completion. Claims abandoned by an interrupted process
become recoverable after a bounded interval.

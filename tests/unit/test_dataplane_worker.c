...
static void test_processing(void **state)
{
    ...
    (void)state;
    assert_int_equal(jg_dataplane_worker_create(store, 0U, NULL, NULL, &worker),
                     0);
...
static void test_arguments(void **state)
{
    ...
    (void)state;
    assert_int_equal(jg_dataplane_worker_create(store, 1U, NULL, NULL, &worker),
                     -EINVAL);
    assert_int_equal(
        jg_dataplane_worker_create(store, 0U, &limits, NULL, &worker), -EINVAL);
    jg_fragment_limits_default(&fragment_limits);
    fragment_limits.max_datagrams = 0U;
    assert_int_equal(
        jg_dataplane_worker_create(store, 0U, NULL, &fragment_limits, &worker),
        -EINVAL);
    jg_fragment_limits_default(&fragment_limits);
    ...
    assert_int_equal(
        jg_dataplane_worker_create(store, 0U, NULL, &fragment_limits, &worker),
        -ERANGE);
    assert_int_equal(jg_dataplane_worker_create(store, 0U, NULL, NULL, NULL),
                     -EINVAL);

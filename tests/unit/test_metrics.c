...
static void test_bounded_rendering(void **state)
{
    ...
    (void)state;
    assert_int_equal(jg_metrics_render(&stats, probe, sizeof(probe), &required),
                     -ENOSPC);
    ...
    assert_int_equal(jg_metrics_render(&stats, output, required + 1U, &written),
                     0);
    assert_int_equal(written, required);
    free(output);

    assert_int_equal(jg_metrics_render(NULL, probe, sizeof(probe), &written),
                     -EINVAL);
    assert_int_equal(jg_metrics_render(&stats, NULL, 0U, &written), -EINVAL);
}

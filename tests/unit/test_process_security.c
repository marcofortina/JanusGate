...
/** @brief Verify an allowlisted call and rejection of `ptrace`. */
static void test_seccomp_allowlist(void **state)
{
    pid_t child = -1;
    int status = 0;
    ...
    if (child == 0) {
        if (jg_process_harden() != 0 ||
            jg_process_apply_seccomp(JG_PROCESS_PROFILE_WEB) != 0 ||
            getpid() <= 0) {
            _exit(1);
        }
        errno = 0;
...
static void test_invalid_profiles(void **state)
{
    ...
    assert_int_equal(jg_process_restrict_capabilities(invalid_profile),
                     -EINVAL);
    assert_int_equal(jg_process_apply_seccomp(invalid_profile), -EINVAL);
...
int jg_test_process_security(void)
{
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_seccomp_allowlist),
